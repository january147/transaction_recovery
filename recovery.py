...
class LogEntry():
    ...
    @staticmethod
    def parse(log_entry):
        lsn, _type, trans_id, prelsn, seg_offset, undo_nxt_lsn, redo_info, undo_info = log_entry
        redo_info = base64.b64decode(redo_info)
        undo_info = base64.b64decode(undo_info)
        log_entry_ob = LogEntry(_type, trans_id, lsn, prelsn, seg_offset, undo_nxt_lsn, redo_info, undo_info)
        return log_entry_ob
...
class LogManager():
    ...
    def init(self):
        try:
            self.log_db.execute('DROP TABLE log')
            self.log_db.execute('DROP TABLE master_rec')
        except:
            pass
        self.create_log_table()
        self.create_master_rec()
...
class BufferManager():
    ...
    def write_segment(self, lsn:int, offset:int, data):
        if offset not in self.dst.data.keys():
            self.dst.insert(offset, lsn)
    ...
    def init(self):
        os.system('dd if=/dev/zero of=disk.img bs=4096 count=1024')
...
class TransactionManager():
    ...
    def analysis(self):
        master_rec = logm.get_master_rec()
        ...
        while log_rec != None:
            ...
            if _type == LogEntry.UPDATE or _type == LogEntry.CLR:
                ...
                if len(redo_info) != 0 and seg_offset not in bufm.dst.data.keys():
    ...
    def redo_from_lsn(self, lsn:int):
        ...
        while log_rec is not None:
            lsn, _type, trans_id, prelsn, seg_offset, undo_nxt_lsn, redo_info, undo_info = log_rec 
            log_rec_entry = LogEntry.parse(log_rec)
            if log_rec_entry.type == LogEntry.UPDATE or log_rec_entry.type == LogEntry.CLR:
                if len(redo_info) != 0:
                    bufm.write_segment(lsn, seg_offset, log_rec_entry.redo_info)
            log_rec = cursor.fetchone()
    ...
    def total_undo(self):
        ...
        for trans_id in self.tct.data.keys():
            ...
            while tc_undo_nxt_lsn != -1:
                ...
                elif _type == LogEntry.UPDATE and len(undo_info) != 0:
                    
                    bufm.write_segment(logm.read_lsn(), seg_offset, log_rec_entry.undo_info)
                    # record a CLR when undo a log entry
                    log_entry = LogEntry(LogEntry.CLR, trans_id, prelsn=last_lsn, redo_info=log_rec_entry.undo_info, undo_nxt_lsn=prelsn)
            ...
            logm.log(log_entry)
            self.tct.data[trans_id][0] = TransactionTable.COMMIT
        self.tct.data.clear()


    def doUpdateTranscation(self, segment_list, data_list, crash = False):
    ...
    def restart_recovery(self):
        redo_lsn = self.analysis()
        self.redo_from_lsn(redo_lsn)
        self.total_undo()
        bufm.dump()
